...
| # | Regla | Relación ArchiMate |
|---|---|---|
| 1 | El **Service Domain sirve** a la **capacidad L2** | Service Domain → *serving* → Capability L2 |
| 2 | La **capacidad L2 compone** a la **capacidad L1** | Capability L1 → *composition* → Capability L2 |
| 3 | El **proceso realiza** una capacidad — **puede ser L1 o L2** | Business Process → *realization* → Capability |
| 4 | El **proceso se compone** de **actividades** | Business Process → *composition* → Business Process (actividad) |
| 5 | La **actividad usa** un **Service Domain** | Service Domain → *serving* → actividad |

```
        Capacidad L1
             ▲
             │  composición  (la L1 se compone de capacidades L2)
        Capacidad L2
             ▲
             │  sirve
        Service Domain  (BIAN)
         ...
         Actividad
             ▲
             │  composición  (el proceso se compone de actividades)
          Proceso  ───── realiza ─────►  Capacidad (L1 o L2)
```

**Lo que hace potente esta cadena** es que se cierra sobre sí misma: el proceso **realiza** una capacidad, esa capacidad **es servida** por Service Domains, y las **actividades** de ese mismo proceso **usan** esos Service Domains. Por eso el proceso deja de "estar automatizado por un sistema" y pasa a **orquestar dominios de servicio reutilizables**, que es exactamente la forma de los *Business Scenarios* de BIAN.

**Secuencia pedagógica.** En el **Módulo 2** se modela el proceso, sus actividades y la realización proceso → capacidad (L1 y L2). En los **Módulos 3–4** se incorporan los Service Domains: sirven a las capacidades L2 y las actividades pasan a usarlos. La columna «Capacidad L2» de cada SIPOC es precisamente el punto de anclaje donde entrarán.
